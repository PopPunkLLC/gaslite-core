...
contract GasliteNFT is ERC721A, Ownable2Step {
    ...
    function whitelistMint(bytes32[] calldata _proof, uint256 _amount) external payable {
        assembly {
            // if (!live)
            if iszero(and(shr(240, sload(11)), 0xff)) {
                mstore(0x00, 0x0aea1c5d) // revert MintNotLive();
                revert(0x1c, 0x04)
            }

            // if (block.timestamp < whitelistOpen || block.timestamp > whitelistClose)
            let s12 := sload(12)
            if or(lt(timestamp(), and(shr(128, s12), 0xffffffffffffffff)), gt(timestamp(), shr(192, s12))) {
                mstore(0x00, 0x1e94bf2d) // revert WhitelistNotLive();
                revert(0x1c, 0x04)
            }
        }
        ...
        assembly {
            // if (minted > maxWhitelistMint)
            if gt(minted, and(sload(12), 0xffffffffffffffff)) {
                mstore(0x00, 0x024fcb21) // revert MintExceeded();
                revert(0x1c, 0x04)
            }
        }
        ...
        if (!MerkleProofLib.verifyCalldata(_proof, whitelistRoot, keccak256(abi.encodePacked(msg.sender)))) {
            assembly {
                mstore(0x00, 0xacab000c) // revert WhitelistMintUnauthorized();
                revert(0x1c, 0x04)
            }
        }

        if (msg.value != _amount * whitelistPrice) {
            assembly {
                mstore(0x00, 0xcd1c8867) // revert InsufficientPayment();
                revert(0x1c, 0x04)
            }
        }
    ...
    function publicMint(uint256 _amount) external payable {
        assembly {
            ...
            // if (block.timestamp < whitelistClose)
            if lt(timestamp(), shr(192, sload(12))) {
                mstore(0x00, 0x10d3e4c9) // revert PublicMintNotLive();
                revert(0x1c, 0x04)
            }
        }
        ...
        uint256 minted = _numberMinted(msg.sender) + _amount;
        assembly {
            // if (minted > maxPublicMint)
            if gt(minted, and(shr(64, sload(12)), 0xffffffffffffffff)) {
                mstore(0x00, 0x024fcb21) // revert MintExceeded();
                revert(0x1c, 0x04)
            }
        }
