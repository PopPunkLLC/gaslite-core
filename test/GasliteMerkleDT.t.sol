...
contract GasliteMerkleDTTest is Test {
    GasliteMerkleDT drop;
    Token token;
    ...
    function setUp() external {
        ...
        vm.startPrank(owner);
        drop = new GasliteMerkleDT(address(token), root);
        drop.toggleActive();
        vm.stopPrank();
    ...
    function test_claimInsufficientBalance() external {
        ...
        bytes32[] memory proof = merkle.getProof(data, 51);
        vm.prank(recipients[50]);
        vm.expectRevert(GasliteMerkleDT.InsufficientBalance.selector);
        drop.claim(proof, amount);
    }
